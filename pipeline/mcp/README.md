...
- `tools/list` returns the six pipeline wrappers and their input schemas.
- `tools/call` shells out to the existing pipeline scripts and returns JSON text content.

권장 시작점은 `unity_rag.doctor`다. 이 tool은 provider/auth, Unity 프로젝트 구조, catalog 존재 여부, gateway health, Unity MCP HTTP Local custom tool/resource 노출 상태를 한 번에 점검한다.
