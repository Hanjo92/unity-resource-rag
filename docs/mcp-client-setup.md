...
Gemini / Antigravity / Claude / Claude Code preset은 MCP 인자에서 provider만 바꾸면 된다. 기본 인증은 각각 Google API key, Google OAuth / gcloud access token, Anthropic API key, Claude Code bearer token / `~/.claude/.credentials.json`이다.
