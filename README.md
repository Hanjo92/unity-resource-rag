...
2. 가장 쉬운 시작점은 `unity-resource-rag`의 `unity_rag.start_ui_build`다. 이 tool은 `doctor`를 먼저 실행하고, reference가 있으면 `run_first_pass_ui_build`, 없으면 `run_catalog_draft_ui_build`를 자동 선택한다.
3. 세부 흐름을 직접 보고 싶다면 `unity_rag.doctor`, `unity_rag.run_first_pass_ui_build`, `unity_rag.run_catalog_draft_ui_build`를 각각 따로 호출해도 된다.
