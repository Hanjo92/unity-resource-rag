...
4. reference 이미지가 아직 없으면 `unity_rag.run_catalog_draft_ui_build`로 catalog 기반 popup draft부터 띄울 수 있다.
5. apply 이후 screenshot이 필요하면 `manage_camera`를 실행한다.
...
reference 없이 catalog 기반 draft부터 시작하고 싶다면 다음 tool을 쓰면 된다.
